...
이 리포에는 두 가지 Python 버전용 wheels가 포함되어 있습니다:

| 폴더 | Python 버전 | 플랫폼 |
|------|------------|--------|
| `wheels/` | 3.14 | Windows x64 |
| `wheels_cp312/` | 3.12 | Windows x64 |
...
# 4) 오프라인 설치 — Python 3.12인 경우
pip install --no-index --find-links=wheels_cp312/ -r requirements.lock

# Python 3.14인 경우 (이 PC와 동일한 버전)
...
```powershell
# 예: Python 3.11용
pip download -r requirements.lock -d wheels_cp311/ --platform win_amd64 --python-version 3.11 --only-binary=:all:
```

실패하면 (source-only 패키지가 있을 경우):
```powershell
pip download -r requirements.lock -d wheels_cp311/
```
...
| 증상 | 원인 / 해결 |
|------|------------|
| `No matching distribution found` | Python 버전 불일치. `python --version`으로 확인 후 맞는 wheels 폴더 사용 |
| `Could not find a version that satisfies` | wheels 폴더 경로가 틀림. `--find-links=` 경로 확인 |
...
> `docker build` 명령은 `Dockerfile`을 읽어서 Python 3.12 + 모든 패키지가 설치된 리눅스 환경을 만듭니다. `wheels_cp312/` 폴더의 패키지를 오프라인으로 설치하므로 빌드 시에도 PyPI 접속이 불필요합니다.
...
| `No matching distribution found` | Python 버전 불일치. 맞는 wheels 폴더 사용 |
| 임베딩 차원 불일치 오류 | `rag_storage/` 삭제 후 재실행 |
| Ollama에서 JSON 파싱 실패 | 모델이 너무 작음. 14B 이상으로 교체 |
