...
| `lightrag_onprem_demo.py` | **사내 폐쇄망** 전용 데모 (커스텀 헤더 인증 + FlagEmbedding) |
| `lightrag_ollama_demo.py` | **Ollama 로컬 LLM** 기반 데모 (API 키 불필요) |
| `requirements.txt` | 느슨한 의존성 (인터넷 환경용) |
| `requirements_onprem.txt` | 폐쇄망 환경 버전 고정 (Python 3.10 + FlagEmbedding) |
| `requirements.lock` | 기존 버전 고정 (참고용) |
| `models/bge-m3/` | HuggingFace bge-m3 모델 파일 위치 (별도 다운로드 필요) |
...
|------|------------|--------|-----------|
| `wheels_cp310/` | 3.10 | Windows x64 | LightRAG + FlagEmbedding + PyTorch(CPU) |

> FlagEmbedding(BAAI/bge-m3)을 로컬에서 직접 실행하여 Ollama 서빙 경로의 NaN 문제를 해결합니다.
...
# 5) bge-m3 임베딩 모델 배치
#    인터넷 PC에서 미리 다운로드한 models/bge-m3/ 폴더를 복사
...
#   ONPREM_EMBED_USE_GPU=true (GPU 없으면 false)
...
### GPU 환경에서 PyTorch CUDA 버전 사용하려면?

`wheels_cp310/`에 포함된 torch는 CPU 버전(114MB)입니다.
GPU(CUDA)를 사용하려면 인터넷 PC에서 별도로 다운로드하세요:

```powershell
# CUDA 12.1 예시
pip download torch --index-url https://download.pytorch.org/whl/cu121 -d wheels_cp310/ --python-version 3.10 --platform win_amd64 --only-binary=:all: --no-deps
# 기존 CPU 버전 삭제
del wheels_cp310\torch-*-cp310-cp310-win_amd64.whl
```
...
| `config.json이 없습니다` | models/bge-m3/ 폴더에 모델 파일이 없음. HuggingFace에서 다운로드 필요 |
| `CUDA not available` | `.env.onprem`에서 `ONPREM_EMBED_USE_GPU=false`로 변경 (CPU 모드) |
...
| FlagEmbedding 모델 경로 에러 | `models/bge-m3/`에 모델 파일 배치 확인 |
